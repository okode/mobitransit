...
- (id)initWithHost:(NSString *)theHost 
		  ...
		  delegate:(id<CRVStompClientDelegate>)theDelegate
	   autoconnect:(BOOL) autoconnect
		   withTSL:(NSDictionary *) tslParams{
	if(self = [super init]) {
...
- (void)onSocket:(AsyncSocket *)sock willDisconnectWithError:(NSError *)err {
	if([[self delegate] respondsToSelector:@selector(stompClientConnectionError:)]) {
		[[self delegate] stompClientConnectionError:err];
	}
}
